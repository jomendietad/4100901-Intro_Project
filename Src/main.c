...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_heartbeat_time >= 1500) // Cada 1500 ms
    {
    ...
    */
    ...
    if (button_flag == 1)
    {
      ...
      HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_SET); // Cambia el estado del LED LD2 (opcional)
      // 2. Calcular cuándo apagarlo (1 segundo desde ahora)
      led_ext_off_time = HAL_GetTick() + 1000;

      // 3. Enviar mensaje por UART
      sprintf(tx_buffer, "Boton presionado, leds encendidos.\r\n");
    ...
    if (led_ext_off_time != 0 && HAL_GetTick() >= led_ext_off_time)
    {
      ...
      HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_RESET); // Cambia el estado del LED LD2 (opcional)
      // 2. Enviar mensaje por UART (opcional)
      sprintf(tx_buffer, "Leds apagados (tiempo de espera superado).\r\n");
      HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
